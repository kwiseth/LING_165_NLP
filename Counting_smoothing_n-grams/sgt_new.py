...
def count_freq(freq_dict):
    ...
    """
    d = {}
    print('inside the count_freq(freq_dict) method \n')
    for entry in freq_dict.keys():
        freq = freq_dict[entry]
        if freq not in d:
            d[freq] = 0
        d[freq] += 1
    print('\nfrequency of freq is equal to ', d, '\n')
    return d
...
def fix_zero(freq_freq_dict):
    ...
    c_list = sorted(freq_freq_dict.keys())
...
def linear_regression(freq_freq_dict):
    ...
    if len(log_c) == 1:
        ...
        slope, intercept, _, _, _ = stats.linregress(log_c, log_n_c)
...
def turing(c, c_dict, n):
    """Turing estimate of c, i.e. use c_dict[c] as N_c:
    c* = (c+1)*N_c+1/N_c
    ...
    """
    if c == 0:
        freq_star = (c + 1) * c_dict[c + 1] / n
    else:
        freq_star = (c + 1) * c_dict[c + 1] / c_dict[c]
    return freq_star
...
def var_turing(c, c_dict):
    ...
    prod = 1.0
    prod *= c ** 2
    prod *= float(c_dict[c + 1]) / (c_dict[c] ** 2)
...
def freq_star_dict(c_dict, n, slope, intercept):
    ...
    c_star_dict = {}
    e_c = 'turing'
    for c in [0] + list(c_dict.keys()):
        if c == 0:
            c_star = turing(c, c_dict, n)
        else:
            lgt_c = lgt(c, n, slope, intercept)
            if e_c == 'turing':
                t_c = turing(c, c_dict, n)
                diff = lgt_c - t_c
                if diff <= 1.65 * var_turing(c, c_dict):
                    e_c = 'lgt'
                else:
                    c_star = t_c
            if e_c == 'lgt':
...
def gt_freq(freq_dict):
    ...
    # 2. Define smoothing function to get expected frequency of frequencies.
